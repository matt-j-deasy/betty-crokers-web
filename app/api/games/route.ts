...
const BASE = process.env.GO_SERVER_URL!; // e.g., http://localhost:8080/api/v1

export async function GET(req: NextRequest) {
  const session = await getServerSession(authOptions);
  const token = (session as any)?.token as string | undefined;

  const upstream = await fetch(`${BASE}/games${req.nextUrl.search}`, {
    cache: "no-store",
...
export async function POST(req: NextRequest) {
  ...
  const upstream = await fetch(`${BASE}/games`, {
    method: "POST",
    headers: {
